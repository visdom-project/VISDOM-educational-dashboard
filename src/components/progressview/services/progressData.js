...
const getModuleMapping = (modules) => {
  ...
  modules.forEach((module) => {
    if (module.exercises.length > 0 || module.id === 570) {
...
const getPointsForWeek = (data, moduleId) => {
  const correct_week = data.points.modules.find(
  ...
  data.data.hits.hits.forEach((hit) => {
    hit._source.results.forEach((result) => {
      if (!result.username.includes("redacted")) {
        ...
        result.cumulativePoints = calcCumulatives(
          newModules.map((module) => module.points)
        );
        result.cumulativeExercises = calcCumulatives(
          newModules.map((module) =>
            module.exercises.reduce((sum, exercise) => {
              sum += exercise.submission_count > 0 ? 1 : 0;
              return sum;
            }, 0)
          )
...
const getData = () => {
  const request = axios
    ...
    .then((response) => {
      ...
      const weeks = moduleMapping.map((id, index) => {
        return { week: index + 1 };
      });
      ...
      weeks.forEach((week) => {
        ...
        response.data.hits.hits.forEach((hit) => {
          hit._source.results.forEach((result) => {
            if (!result.username.includes("redacted")) {
              const [weekPoints, weekExercises] = getPointsForWeek(
                result,
                moduleMapping[week.week - 1]
              );
              week[result.student_id] = weekPoints;
              exerciseWeek[result.student_id] = weekExercises;

              weekCumulatively[result.student_id] =
                result.cumulativePoints[week.week - 1];
              exerciseWeekCumulatively[result.student_id] =
                result.cumulativeExercises[week.week - 1];
            }
          });
        });
      ...
      return historyDataService.getHistoryData().then((response) => {
        ...
        Object.keys(historyByWeeks).forEach((weekName) => {
          ...
          keys.forEach((key) => {
            ...
            historyByWeeks[weekName][`avg_${key}`].forEach((gradeValues) => {
              datastructures[key][parseInt(weekName) - 1][
                `avg_${key}_grade_${index}`
              ] = gradeValues;
              index += 1;
            });
          });
          ...
          historyByWeeks[weekName].avg_cum_points.forEach((gradePoints) => {
            cumulativePoints[parseInt(weekName)][
              `avg_cum_points_grade_${index}`
            ] = gradePoints;
            index += 1;
          });
          index = 0;
          historyByWeeks[weekName].avg_cum_exercises.forEach(
            (gradeExercises) => {
              cumulativeExercises[parseInt(weekName)][
                `avg_cum_exercises_grade_${index}`
              ] = gradeExercises;
              index += 1;
            }
          );
        ...
        grades.forEach((grade) => {
          ...
          cumulativeCommits[cumulativeCommits.length - 1][
            `avg_cum_commits_grade_${grade}`
          ] =
            cumulativeCommits[cumulativeCommits.length - 2][
              `avg_cum_commits_grade_${grade}`
            ];
          cumulativeSubmissions[cumulativeSubmissions.length - 1][
            `avg_cum_submissions_grade_${grade}`
          ] =
            cumulativeSubmissions[cumulativeSubmissions.length - 2][
              `avg_cum_submissions_grade_${grade}`
            ];
        });
...
const getCommitData = (response, moduleMapping) => {
  const weeklyPoints = moduleMapping.map((id, index) => {
    return { week: index + 1 };
  });
  const cumulativeResults = moduleMapping.map((id, index) => {
    return { week: index + 1 };
  });

  const submissions = moduleMapping.map((id, index) => {
    return { week: index + 1 };
  });
  const cumulativeSubmissions = moduleMapping.map((id, index) => {
    return { week: index + 1 };
  });
    ...
    hit._source.results.forEach((student) => {
      if (!student.username.includes("redacted")) {
        ...
        weeklyPoints.forEach((weekObject) => {
          const index = student.commits.findIndex(
            (module) =>
              (module.moduleName === "01-14"
                ? 14
                : parseInt(module.module_name)) === parseInt(weekObject.week)
          );
          const commitSum =
            index < 0
              ? 0
              : student.commits[index].projects.reduce(
                  (sum, project) => sum + project.commit_count,
                  0
                );
        ...
        cumulativeResults.forEach((weekObject) => {
          weekObject[student.student_id] =
            cumulativeCommits[parseInt(weekObject.week) - 1];
        });
        ...
        submissions.forEach((weekObject) => {
          ...
          const submissionCount =
            correctModule !== undefined ? correctModule.submission_count : 0;
        ...
        cumulativeSubmissions.forEach((weekObject) => {
          weekObject[student.student_id] =
            cumulativeSubs[parseInt(weekObject.week) - 1];
        });
      }
    });
  });
